...
export(int) var obstacles_in_the_middle
...
func _ready():
	...
	populate_with_obstacles(1*obstacles_on_screen-5,rect)
	populate_with_bottles(2*bottles_on_screen-1,rect)
...
func _input(event):
	if not is_game_over:
		...
		if event.is_action_type() && event.is_pressed() && !event.is_echo():
			get_tree().change_scene("res://scenes//MainMenu.tscn")
...
func populate_with_obstacles(number_of_obstacles, rect):
	...
	for i in range(0, obstacles_in_the_middle):
		var random_index = randi() % tile_names.size()
		var tilemap = get_node("/root/main/TileMap")
		var tile_index = tilemap.get_tileset().find_tile_by_name( tile_names[random_index] )
